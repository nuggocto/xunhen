...
are unavailable. Repository-local `core.attributesFile` overrides are refused;
put global attribute settings in user Git configuration. External-filter paths
remain indeterminate. Untracked files are outside this view.
...
Optional `[limits]` settings `git_timeout_ms` (1–30,000) and
`git_output_bytes` (64–67,108,864) lower the limits for every Git command,
including loading, refresh, and patch reads. Output limits apply separately to
stdout and stderr, alongside each operation's smaller compiled caps. Repository
commands default to 30 seconds and at most 64 MiB of stdout; stderr is capped at
64 KiB. The version probe defaults to 2 seconds and 16 KiB per stream and never
exceeds 5 seconds or 64 KiB, even with larger configured limits.
